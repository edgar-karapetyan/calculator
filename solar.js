...
function calculateLoanWithDelay() {
    ...
    loanCalculationTimer = setTimeout(calculateLoan, 800); // Задержка 800ms
}
...
function calculateEnergyWithDelay() {
    ...
    energyCalculationTimer = setTimeout(calculateEnergy, 800); // Задержка 800ms
}
...
function createChart(loanAmount, monthlyInterestRate, loanTermMonths, paymentType) {
    ...
    if (labels[labels.length - 1] !== `Месяц ${loanTermMonths}`) {
        ...
        if (paymentType === 'annuity') {
            ...
            const principal = loanAmount / loanTermMonths;
            const interest = principal * monthlyInterestRate; // Для последнего месяца
            paymentData.push(principal + interest);
            principalData.push(principal);
            interestData.push(interest);
        }
...
// Данные для таблицы солнечных панелей из предоставленных PDF
const solarData = [
    // LONGi 11% հիբրիդային 24.09.25.pdf
    ...
    // LONGi 11% ցանցային 24.09.25.pdf
    ...
    // SWISS 11% հիբրիդային 24.09.25.pdf
    ...
    // SWISS 11% ցանցային 24.09.25.pdf
...
function populateSolarTable() {
    ...
    solarData.forEach(item => {
        ...
        // Определяем тип системы на русском
...
function getSolarRecommendations(monthlyConsumption) {
    const recommendations = solarData.filter(item => 
        item.savings && monthlyConsumption <= item.savings * 1.2
    ).sort((a, b) => a.cost - b.cost);
    
    return recommendations.slice(0, 5); // Возвращаем 5 самых дешевых вариантов
}